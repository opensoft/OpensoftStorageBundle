...
class LocalAdapterConfiguration extends AbstractAdapterConfiguration implements UsageAwareInterface
{
     ...
     * {@inheritdoc}
     */
    protected function doCreateAdapter(array $options): AdapterInterface
    {

//        $adapter = new Local($options['directory'], $options['create'], intval($options['mode'], 8));
        $adapter =  new Local(
            $options['directory'],
            LOCK_EX,
            ...
            [
                'file' => [
                    'public' => intval($options['mode'], 8),
                    'private' => intval($options['mode'], 8)
                ],
                'dir' => [
                    'public' => intval($options['mode'], 8),
                    'private' => intval($options['mode'], 8)
     ...
     * @return array
     */
    public function usage(Storage $storage): array
    {
    ...
    public static function createPermanentUrlResponse(Request $request, string $scheme, string $storageKey, array $adapter, array $storageInfo): Response
    {
        ...
        if ($storageInfo['size_in_bytes'] > 1000000) {
            $response = new RedirectResponse(sprintf(
                '%s://%s/%s/%s',
                $scheme,
                $adapter['http_host'] ?? $request->getHttpHost(), // TODO - remove after all storages have http_host values
