...
class AwsS3AdapterConfiguration extends AbstractAdapterConfiguration
{
     ...
     */
    public function __construct(RouterInterface $router, LoggerInterface $logger, $permanentBaseUrl)
    {
        $this->router = $router;
        $this->logger = $logger;
...
//        $adapter = new AwsS3($service, $options['bucket_name'], [], true);
        ...
        return new AwsS3Adapter($service, $options['bucket_name']);
    }
    ...
    public function getUrl(StorageFile $file, string $referenceType = StorageUrlResolverInterface::ABSOLUTE_URL): string
    {
        ...
        $hostname = $adapterOptions['cname'] ?? sprintf('%s.s3.amazonaws.com', $adapterOptions['bucket_name']);
